...
(function(_e) {
  "use strict";
  if(!QIMEN_STAR.hasOwnProperty('GetBazi')) return;
  if(!QIMEN_STAR.hasOwnProperty('CalCurrentJiqi')) return;
  // 
  /**
   * 計紫白飛星年盤
   */
  function CalYear(yy,mm,dd,hh,ii,ss) {
    var jiqi = QIMEN_STAR.CalCurrentJiqiIdx(yy,mm,dd,hh,ii,ss);
    var bazi = QIMEN_STAR.GetBazi(yy,mm,dd,hh,ii,ss);
    if(jiqi > 21) yy -= 1; 
